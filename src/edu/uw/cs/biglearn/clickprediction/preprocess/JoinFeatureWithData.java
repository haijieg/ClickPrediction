 ...
 * AdvertiserID | Depth | Position | QueryID | KeywordID | TitleID | DescriptionID | UserID
 * 1. click: number of clicks.
 * 2. Impression: number of impressions.
 * 3. DisplayURL: a property of the ad. The URL is shown together
 ...
 * 5. AdvertiserID: a property of the ad. Some advertisers consistently optimize their ads, so the title and
 * description of their ads are more attractive than those of others.
 * 6.Depth: a property of the session: Number of total ads impressed.
 * 7. Position: a property of an ad in a session: the rank.
 * 8. QueryID: id of the query. This id is a zero-based integer value. It is the key of the data file
 * 'queryid_tokensid.txt'.
 * 9. KeywordID: a property of ads. This is the key of 'purchasedkeyword_tokensid.txt'.
 * 10. TitleID: a property of ads. This is the
 * key of 'titleid_tokensid.txt'.
 * 11. DescriptionID: a property of ads. This is the key of 'descriptionid_tokensid.txt'.
 * 12. UserID This is the key of 'userid_profile.txt'. When we cannot identify the user, this field has a
 * special value of 0.
 * 
 * The filtered data schema after join: click | impression | depth | position |
query tokens | keyword tokens | title tokens | description tokens | user tokens
 */
	...
	public static ArrayList<String> processLine(String line, boolean hasLabel) {
		...
		if (descriptionid < Features.descriptionFeature.size()) {
			filteredTokens.add(StringUtil.implode(
					Features.descriptionFeature.get(descriptionid).iterator(),
					","));
		} else {
			filteredTokens.add("-1");
